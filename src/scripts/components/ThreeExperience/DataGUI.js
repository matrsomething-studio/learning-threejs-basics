...
export default class ThreeGUI {
    constructor(base) {
        ...
        this.base.meshes.forEach((mesh, indx) => {
            let folder = this.GUI.addFolder(
                `${mesh.geometry.type} 00${++indx}`
            );

            cords.forEach((cord) => {
                folder
                    .add(mesh.position, cord, -1, 1, 0.01)
                    .name(`Translate ${cord}`);
                folder
                    .add(mesh.rotation, cord, 0, Math.PI * 2, 0.01)
                    .name(`Rotate ${cord}`);
        ...
        this.base.lights.forEach((light, indx) => {
            ...
            cords.forEach((cord) => {
                if (!light.type.includes('Ambient')) {
                    folder
                        .add(light.position, cord, -1, 1, 0.01)
                        .name(`Translate ${cord}`);
                }
            });
